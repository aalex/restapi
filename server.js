/**
 * This files is our main entry for our web server application.
 * 
 * port: 8080
 * Routes:
 * - POST /setup - Middleware before authentification
 * - POST /authenticate - Check password and username is found. Provide token.
 */
...
router.route('/users/:users_id').get(function(req, res) {
...
router.route('/users/:users_id').put(function(req, res) {
    User.findById(req.params.users_id, function(err, user) {
        if (err) {
            res.send(err);
        }
...
router.route('/users/:users_id').delete(function(req, res) {
    User.remove({
